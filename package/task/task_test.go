...
func TestCreateTask(t *testing.T) {
	...
	taskInstance := task.CreateTask("Test Task", params)

	// Since ID is generated internally, we can't predict its exact value.
	// However, we can check that it's greater than zero.
	if taskInstance.ID <= 0 {
		t.Errorf("Expected task ID to be greater than 0, but got %d", taskInstance.ID)
	}
...
func TestUpdateStatus(t *testing.T) {
	taskInstance := task.CreateTask("Update Status Task", nil)
...
func TestUpdateOwner(t *testing.T) {
	taskInstance := task.CreateTask("Update Owner Task", nil)
...
func TestUpdateParameters(t *testing.T) {
	taskInstance := task.CreateTask("Update Parameters Task", map[string]interface{}{
		"key1": "value1",
	})
...
func TestUpdateResult(t *testing.T) {
	taskInstance := task.CreateTask("Update Result Task", nil)
...
func TestGetInfoString(t *testing.T) {
	taskInstance := task.CreateTask("Display Task", map[string]interface{}{
		"key1": "value1",
	})
